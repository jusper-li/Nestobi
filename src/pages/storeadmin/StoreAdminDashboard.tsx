...
function resolveSalesRange(
  range: keyof typeof SALES_RANGE_LABELS,
  start: string,
  end: string,
) {
...
export default function StoreAdminDashboard() {
  ...
  const [redemptionForm, setRedemptionForm] =
    useState<RedemptionForm>(emptyRedemptionForm);
  ...
  const [salesRangePreset, setSalesRangePreset] =
  ...
  const assignedStoreIds = useMemo(
    () => storeAssignments.map((item) => item.store_location_id),
    [storeAssignments],
  );
  const visibleStores = useMemo(
    () =>
      isElevated
        ...
        : stores.filter(
            (store) =>
              (vendorId && store.vendor_id === vendorId) ||
  ...
  const canManageStore = (
    storeId: string,
    permission:
      "any" | "info" | "products" | "inventory" | "points" | "sales" = "any",
  ) => {
    const store = stores.find((item) => item.id === storeId);
    const isVendorOwner = !!store && !!vendorId && store.vendor_id === vendorId;
    return (
      isElevated || isVendorOwner || hasStorePermission(storeId, permission)
    );
  };
  const canEditStoreInfo =
    !!selectedStoreId && canManageStore(selectedStoreId, "info");
  const canEditProducts =
    !!selectedStoreId && canManageStore(selectedStoreId, "products");
  const canEditInventory =
    !!selectedStoreId && canManageStore(selectedStoreId, "inventory");
  const canEditPoints =
    !!selectedStoreId && canManageStore(selectedStoreId, "points");
  const canEditSales =
    !!selectedStoreId && canManageStore(selectedStoreId, "sales");
  const salesRangeLabel = SALES_RANGE_LABELS[salesRangePreset].label;
  const salesStats = useMemo(() => {
    const totalRevenue = salesRecords.reduce(
      (sum, record) => sum + Number(record.revenue_amount || 0),
      0,
    );
    const count = salesRecords.length;
    const avgRevenue = count ? totalRevenue / count : 0;
    const highestRecord = salesRecords.reduce<StoreDailySale | null>(
      (best, record) => {
        if (!best) return record;
        return Number(record.revenue_amount || 0) >
          Number(best.revenue_amount || 0)
          ? record
          : best;
      },
      null,
  ...
  const moduleNav = [
    ...
    {
      key: "sales" as const,
      label: pick("銷售成績", "Sales performance", "売上実績", "매출 성과"),
      hint: pick(
        "每日輸入營業額與區間分析",
        "Daily input and range stats",
        "日別入力と期間分析",
        "일별 입력 및 구간 통계",
      ),
    },
    {
      key: "recent" as const,
      label: pick("最近紀錄", "Recent records", "最近の記録", "최근 기록"),
      hint: pick(
        "快速查看最新資料",
  ...
  const activeModuleItem =
    moduleNav.find((item) => item.key === activeModule) ?? moduleNav[0];

  const loadStores = async (currentVendorId: string | null) => {
    let query = supabase
      .from("store_locations")
      .select(
        "id,vendor_id,name,name_en,slug,city,district,address,phone,hours,image_url,map_url,sort_order,is_active,source_url,source_image_url,manager_notes,created_at,updated_at",
      )
    ...
    return ((data || []) as StoreLocation[]).map((row, index) =>
      normalizeStoreLocation(row, index),
    );
  };
  ...
  const loadManagers = async (storeId: string) => {
    ...
    const { data, error } = await supabase
      ...
      .select(
        "id,store_location_id,user_id,role,can_manage_store_info,can_manage_products,can_manage_inventory,can_manage_points,can_manage_sales,is_active,created_at,updated_at",
      )
  ...
  const loadStoreData = async (storeId: string) => {
    const salesRange = resolveSalesRange(
      salesRangePreset,
      salesRangeStart,
      salesRangeEnd,
    );
    const [
      ...
      supabase
        .from("products")
        .select(
          "id,category_id,vendor_id,store_location_id,name,description,price,image_url,stock_quantity,is_active,sku,origin,roast_level,processing_method,flavor_notes,tags,created_at,updated_at",
        )
        .eq("store_location_id", storeId)
      ...
      supabase
        ...
        .select(
          "id,store_location_id,product_id,movement_type,quantity,unit_cost,supplier_name,invoice_no,purchase_date,note,created_by,created_at,updated_at,products(id,name,price,stock_quantity,is_active,sku)",
        )
        .eq("store_location_id", storeId)
        .order("purchase_date", { ascending: false })
        .order("created_at", { ascending: false }),
      supabase
        .from("store_point_redemptions")
        .select(
          "id,store_location_id,user_id,points_used,discount_amount,reference_type,reference_id,note,used_at,created_by,created_at,updated_at",
        )
        .eq("store_location_id", storeId)
      ...
      supabase
        ...
        .select(
          "id,store_location_id,sales_date,revenue_amount,note,recorded_by,created_at,updated_at",
        )
        .eq("store_location_id", storeId)
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      setStores(storeRows);
      const nextStoreId = storeRows.some(
      ...
      if (nextStoreId) {
        ...
        if (current) {
          ...
          await Promise.all([
            loadStoreData(nextStoreId),
            loadManagers(nextStoreId),
          ]);
        }
      }
    } catch (error) {
      setMessage({
        ...
        text:
          error instanceof Error
            ...
            : pick(
                "載入門市管理資料失敗",
                "Failed to load store admin data",
                "店舗管理データの読み込みに失敗しました。",
                "매장 관리 데이터를 불러오지 못했습니다.",
              ),
      });
    } finally {
      setLoading(false);
  ...
  const handleQrPayload = async (payload: string) => {
    ...
    if (!userId) {
      setQrScannerError(
        "無法辨識這個 QR 內容，請改掃會員碼或直接輸入 User ID。",
      );
      return;
    }
    setQrScannerError(null);
    ...
    try {
      ...
      qrTimerRef.current = window.setInterval(async () => {
        if (
          ...
          !qrVideoRef.current ||
          qrVideoRef.current.readyState < 2
        )
          return;
  ...
  const scanQrFromImage = async (file: File) => {
    ...
    try {
      setQrScannerLoading(true);
      setQrScannerError(null);
      const detector = (qrDetectorRef.current ||= new BarcodeDetector({
        formats: ["qr_code"],
      }));
      ...
      setQrScannerLoading(false);
      setQrScannerError(
        error instanceof Error ? error.message : "圖片掃描失敗。",
      );
    } finally {
      setQrScannerLoading(false);
    }
  ...
  const saveStore = async () => {
    ...
    try {
      const payload = {
        ...
        hours: {
          primary: storeForm.hours_primary.trim(),
          secondary: storeForm.hours_secondary.trim() || undefined,
        },
  ...
  const assignManager = async (selected: SearchResult) => {
    ...
    if (!canEditStoreInfo) {
      setMessage({
        ...
        text: pick(
          "只有門市資訊管理者可以指派門市管理員。",
          "Only users with store info permission can assign store managers.",
          "店舗情報を管理できる人だけが店舗管理者を割り当てられます。",
          "매장 정보 권한이 있는 사람만 매장 관리자를 지정할 수 있습니다.",
        ),
      });
      return;
    }
    ...
    if (!canEditProducts) {
      setMessage({
        ...
        text: pick(
          ...
          "この店舗の商品を管理する権限がありません。",
          "이 매장의 상품을 관리할 권한이 없습니다.",
        ),
      });
      return;
    }
  ...
  const saveRedemption = async () => {
    if (
      !selectedStoreId ||
      !redemptionForm.user_id ||
      !redemptionForm.points_used
    )
      return;
    if (!canEditPoints) {
      setMessage({
        ...
        text: pick(
          "你沒有這間門市的點數折抵權限。",
          "You do not have permission to redeem points for this store.",
          "この店舗のポイント利用権限がありません。",
          "이 매장의 포인트 차감 권한이 없습니다.",
        ),
      });
      return;
    }
  ...
  const saveSales = async () => {
    if (
      !selectedStoreId ||
      !salesForm.sales_date ||
      salesForm.revenue_amount === ""
    )
      return;
    if (!canEditSales) {
      setMessage({
        ...
        text: pick(
          "你沒有這間門市的營業額管理權限。",
          "You do not have permission to manage store sales.",
          "この店舗の売上管理権限がありません。",
          "이 매장의 매출 관리 권한이 없습니다.",
        ),
      });
      return;
    }
    ...
    return (
      <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-10 text-center">
        ...
        <p className="mt-2 text-sm text-slate-500">
          {pick(
            "請先請超級管理員建立你的門市權限。",
            "Ask a super admin to assign your store access first.",
            "まずはスーパー管理者に店舗アクセスを割り当ててもらってください。",
            "먼저 최고 관리자에게 매장 접근 권한을 배정해 달라고 요청하세요.",
          )}
        </p>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-2xl bg-white p-5 shadow-sm lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            {selectedStore?.city
              ? `${selectedStore.city} ${selectedStore.district}`
              : ""}
          </p>
        </div>
        <div className="min-w-[260px]">
          <label className="mb-1 block text-xs font-semibold text-slate-500">
            {pick("切換門市", "Switch store", "店舗を切り替える", "매장 전환")}
          </label>
          <select
      ...
      <div className="grid gap-4 md:grid-cols-4">
        {[
          {
            label: pick("商品數", "Products", "商品数", "상품 수"),
            value: products.length,
            icon: Package,
            tone: "bg-amber-50 text-amber-700",
          },
          {
            label: pick("進貨紀錄", "Inventory logs", "在庫記録", "입고 기록"),
            value: movements.length,
            icon: Warehouse,
            tone: "bg-sky-50 text-sky-700",
          },
          {
            label: pick("折抵紀錄", "Redemptions", "利用記録", "차감 기록"),
            value: redemptions.length,
            icon: Coins,
            tone: "bg-emerald-50 text-emerald-700",
          },
          {
            label: pick("管理員", "Managers", "管理者", "관리자"),
          ...
          return (
            ...
            >
              <div
                className={`mb-3 inline-flex h-10 w-10 items-center justify-center rounded-xl ${card.tone}`}
              >
      ...
      <section className="rounded-2xl bg-white p-5 shadow-sm">
        <div className="mb-4 flex items-center gap-2">
          ...
          <div>
            <h2 className="text-lg font-bold text-slate-900">
              {pick("功能列表", "Function list", "機能一覧", "기능 목록")}
            </h2>
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p
                      className={`text-base font-bold ${isActive ? "text-amber-800" : "text-slate-900"}`}
                    >
                      {item.label}
                    </p>
        ...
        <div className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
          <section className="rounded-2xl bg-white p-5 shadow-sm">
            <div className="mb-4 flex items-center gap-2">
              ...
              <div>
                <h2 className="text-lg font-bold text-slate-900">
                  {pick("門市資訊", "Store profile", "店舗情報", "매장 정보")}
                </h2>
                <p className="text-sm text-slate-500">
                  {pick(
                    "可更新名稱、電話、地址與備註。",
                    "Update the store name, contact details, and notes.",
                    "店舗名、連絡先、備考を更新します。",
                    "매장 이름, 연락처, 메모를 수정합니다.",
                  )}
                </p>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <input
                ...
                placeholder={pick("門市名稱", "Store name", "店舗名", "매장명")}
              />
              <input
                value={storeForm.name_en}
                onChange={(e) =>
                  setStoreForm((prev) => ({ ...prev, name_en: e.target.value }))
                }
                className="rounded-xl border border-slate-200 px-3 py-2.5 text-sm"
                placeholder={pick(
                  "英文名稱",
              ...
              <input
                ...
                className="rounded-xl border border-slate-200 px-3 py-2.5 text-sm"
                placeholder={pick("城市", "City", "都市", "도시")}
              />
              <input
                ...
                className="rounded-xl border border-slate-200 px-3 py-2.5 text-sm"
                placeholder={pick("區域", "District", "区", "구")}
              />
              <input
                ...
                placeholder={pick("地址", "Address", "住所", "주소")}
              />
              <input
                ...
                className="rounded-xl border border-slate-200 px-3 py-2.5 text-sm"
                placeholder={pick("電話", "Phone", "電話", "전화")}
              />
              <input
                value={storeForm.hours_primary}
                onChange={(e) =>
                  setStoreForm((prev) => ({
                    ...prev,
                    hours_primary: e.target.value,
                  }))
                }
                className="rounded-xl border border-slate-200 px-3 py-2.5 text-sm"
                placeholder={pick("營業時間", "Hours", "営業時間", "영업시간")}
              />
              <textarea
                value={storeForm.manager_notes}
                onChange={(e) =>
                  setStoreForm((prev) => ({
                    ...prev,
                ...
                rows={3}
                className="md:col-span-2 rounded-xl border border-slate-200 px-3 py-2.5 text-sm"
                placeholder={pick("管理備註", "Notes", "備考", "메모")}
              />
            </div>
            <div className="mt-4 flex items-center justify-between gap-3">
              ...
              <button
                type="button"
                onClick={saveStore}
                disabled={busy || !canEditStoreInfo}
                className="inline-flex items-center gap-2 rounded-xl bg-amber-500 px-4 py-2 text-sm font-semibold text-white disabled:opacity-60"
              >
                {busy ? (
            ...
            <section className="rounded-2xl bg-white p-5 shadow-sm">
              <div className="mb-4 flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    {pick(
                      "搜尋會員後可指派到這間門市。",
                      "Search a member and assign them to this store.",
                      "会員を検索してこの店舗に割り当てます。",
                      "회원을 검색해 이 매장에 지정합니다.",
                    )}
                  </p>
              ...
              {!canManageManagers ? (
                <p className="rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-6 text-sm text-slate-500">
                  {pick(
                    "你目前沒有這間門市的資訊管理權限。",
                    "You currently do not have info management permission for this store.",
                    "この店舗の情報管理権限がありません。",
                    "현재 이 매장의 정보 관리 권한이 없습니다.",
                  )}
                </p>
              ) : (
                <>
                  <div className="grid gap-3 md:grid-cols-[1fr_auto]">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setManagerForm((prev) => ({
                          ...prev,
                          role: e.target.value as StoreLocationManager["role"],
                        }))
                      }
                      className="rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm"
                    >
                      <option value="manager">
                        {pick("管理員", "Manager", "管理者", "관리자")}
                      </option>
                  ...
                  <div className="mt-3 flex flex-wrap gap-3 text-sm text-slate-600">
                    <label className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        checked={managerForm.can_manage_store_info}
                        onChange={(e) =>
                          setManagerForm((prev) => ({
                            ...prev,
                            can_manage_store_info: e.target.checked,
                          }))
                        }
                      />
                    ...
                    <label className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        checked={managerForm.can_manage_products}
                        onChange={(e) =>
                          setManagerForm((prev) => ({
                            ...prev,
                    ...
                    <label className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        checked={managerForm.can_manage_inventory}
                        onChange={(e) =>
                          setManagerForm((prev) => ({
                            ...prev,
                    ...
                    <label className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        checked={managerForm.can_manage_points}
                        onChange={(e) =>
                          setManagerForm((prev) => ({
                            ...prev,
                            can_manage_points: e.target.checked,
                          }))
                        }
                      />
                    ...
                    <label className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        checked={managerForm.can_manage_sales}
                        onChange={(e) =>
                          setManagerForm((prev) => ({
                            ...prev,
                            can_manage_sales: e.target.checked,
                          }))
                        }
                      />
                    ...
                    {managerResults.length === 0 ? (
                      ...
                      managerResults.map((result) => (
                        <button
                          key={result.user_id}
                          type="button"
                          onClick={() => assignManager(result)}
                          disabled={busy}
                          className="flex w-full items-center justify-between rounded-xl border border-slate-200 px-3 py-2.5 text-left text-sm hover:border-amber-300 hover:bg-amber-50 disabled:opacity-60"
                        >
                      ...
                      <div className="divide-y divide-slate-100">
                        {managers.map((manager) => (
                          ...
                          >
                            ...
                            <span
                              className={`rounded-full px-2.5 py-1 text-xs font-semibold ${manager.is_active ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}`}
                            >
                              {manager.is_active
                                ? pick("啟用", "Active", "有効", "활성")
                                : pick("停用", "Inactive", "無効", "비활성")}
                            </span>
                          </div>
                        ))}
                      </div>
          ...
          <section className="rounded-2xl bg-white p-5 shadow-sm">
            <div className="mb-4 flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">
                  {pick(
                    "將折抵點數與使用日期記錄到門市。",
                    "Redemptions are written to the ledger with date and store.",
                    "店舗ごとの利用日とポイントを記録します。",
                    "매장별 사용일과 포인트를 기록합니다.",
                  )}
                </p>
            ...
            <div className="grid gap-3 md:grid-cols-2">
                  ...
                  <div className="mt-2 space-y-2">
                    {memberResults.map((result) => (
                      <button
                        key={result.user_id}
                        type="button"
                        onClick={() => void applyMemberSelection(result)}
                        className="flex w-full items-center justify-between rounded-xl border border-slate-200 px-3 py-2 text-left text-sm hover:border-emerald-300 hover:bg-emerald-50"
                      >
              ...
              <input
                type="number"
                min="1"
                value={redemptionForm.points_used}
                onChange={(e) =>
                  setRedemptionForm((prev) => ({
                    ...prev,
                    points_used: e.target.value,
                  }))
                }
                className="rounded-xl border border-slate-200 px-3 py-2.5 text-sm"
                placeholder={pick(
              ...
              <input
                type="number"
                min="0"
                value={redemptionForm.discount_amount}
                onChange={(e) =>
                  setRedemptionForm((prev) => ({
                    ...prev,
              ...
              <input
                type="datetime-local"
                value={redemptionForm.used_at.slice(0, 16)}
                onChange={(e) =>
                  setRedemptionForm((prev) => ({
                    ...prev,
                    used_at: new Date(e.target.value).toISOString(),
                  }))
                }
                className="rounded-xl border border-slate-200 px-3 py-2.5 text-sm"
              />
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60 md:col-span-2"
              >
                {busy ? (
                  <Loader2 className="h-4 w-4 animate-spin" />
                ) : (
            ...
            <div className="mb-4 flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">
                  {pick(
                    "快速查看商品、進貨與點數折抵。",
                    "Quickly review products, stock intake, and point redemptions.",
                    "商品、入庫、ポイント利用をすばやく確認できます。",
                    "상품, 입고, 포인트 차감을 빠르게 확인합니다.",
                  )}
                </p>
              </div>
            </div>
                    ...
                    products.map((product) => (
                      ...
                      >
                        <div>
                          ...
                          <p className="text-xs text-slate-400">
                            {product.sku || "SKU"} · {product.stock_quantity}
                          </p>
                        </div>
                        <p className="font-semibold text-slate-900">
      ...
      {(activeModule === "overview" || activeModule === "sales") && (
        <section className="rounded-2xl bg-white p-5 shadow-sm">
          <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500">
                  {pick(
                    "可依日期區間查看趨勢，並由門市人員每日輸入當日營業額。",
                    "View performance by date range and let staff enter each day's revenue.",
                    "日付範囲で実績を確認し、スタッフが毎日の売上を入力できます。",
                    "날짜 구간별로 실적을 보고, 매장 직원이 매일 매출을 입력할 수 있습니다.",
                  )}
                </p>
              </div>
            </div>
          ...
          <div className="grid gap-3 md:grid-cols-4">
            ...
            <div className="rounded-2xl bg-slate-50 p-4">
              <p className="text-xs font-semibold text-slate-500">
                {pick("總營業額", "Total revenue", "総売上", "총 매출")}
              </p>
              <p className="mt-1 text-lg font-bold text-slate-900">
            ...
            <div className="rounded-2xl bg-slate-50 p-4">
              ...
              <p className="mt-1 text-lg font-bold text-slate-900">
                {formatCurrency(salesStats.avgRevenue)}
              </p>
            </div>
            <div className="rounded-2xl bg-slate-50 p-4">
              <p className="text-xs font-semibold text-slate-500">
                {pick("最高單日", "Highest day", "最高日売上", "최고 매출일")}
              </p>
              <p className="mt-1 text-lg font-bold text-slate-900">
                {formatCurrency(salesStats.highestRevenue)}
              </p>
            </div>
          </div>
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-[0.95fr_1.05fr]">
            <div className="rounded-2xl border border-slate-200 p-4">
              ...
              <div className="space-y-3">
                ...
                <input
                  type="number"
                  min="0"
                  value={salesForm.revenue_amount}
                  onChange={(e) =>
                    setSalesForm((prev) => ({
                      ...prev,
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setSalesForm((prev) => ({ ...prev, note: e.target.value }))
                  }
                  rows={3}
                  className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm"
                  placeholder={pick(
                ...
                <button
                  type="button"
                  onClick={saveSales}
                  disabled={busy || !canEditSales}
                  className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60"
                >
                  {busy ? (
                    <Loader2 className="h-4 w-4 animate-spin" />
                  ) : (
            ...
            <div className="rounded-2xl border border-slate-200 p-4">
              <div className="mb-3 flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-slate-900">
                  {pick("區間明細", "Range details", "期間明細", "기간별 내역")}
                </h3>
              </div>
              <div className="max-h-80 space-y-2 overflow-y-auto">
                {salesRecords.length === 0 ? (
                  <div className="rounded-xl border border-dashed border-slate-200 px-4 py-6 text-sm text-slate-400">
                    {pick(
                      "這個區間內還沒有營業額資料。",
                      "No sales records in this range yet.",
                      "この期間の売上データはまだありません。",
                      "이 기간에는 아직 매출 데이터가 없습니다.",
                    )}
                  </div>
                ) : (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <p className="text-base font-bold text-slate-900">
                          {formatCurrency(Number(record.revenue_amount || 0))}
                        </p>
                      </div>
                    </div>
                  ))
          ...
          <div className="w-full max-w-2xl overflow-hidden rounded-3xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
              <div>
                ...
                <p className="text-sm text-slate-500">
                  {pick(
                    "請把會員 QR 放在鏡頭前，或上傳 QR 圖片。",
                    "Hold the member QR in front of the camera, or upload an image.",
                    "会員 QR をカメラ前にかざすか、画像をアップロードしてください。",
                    "회원 QR을 카메라 앞에 두거나 이미지를 업로드해 주세요.",
                  )}
                </p>
              </div>
              <button
            ...
            <div className="grid gap-4 p-5 lg:grid-cols-[minmax(0,1.35fr)_minmax(0,0.9fr)]">
              ...
              <div className="space-y-3">
                ...
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  ...
                  <ul className="mt-2 space-y-2 text-sm text-slate-600">
                    ...
                    <li>
                      •{" "}
                      {pick(
                        "含有 user_id / member_id 的網址或 JSON",
                        "A URL or JSON containing user_id / member_id",
                        "user_id / member_id を含む URL または JSON",
                        "user_id / member_id가 포함된 URL 또는 JSON",
                      )}
                    </li>
                    <li>
                      •{" "}
                      {pick(
                        "若 QR 掃不到，也可用下方搜尋會員",
                        "If scan fails, use the member search below.",
                        "読み取れない場合は下の会員検索を使えます。",
                        "스캔이 실패하면 아래 회원 검색을 사용하세요.",
                      )}
                    </li>
                  </ul>
                </div>
                <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-4 text-sm text-slate-500">
                  {pick(
                    "掃描後會自動帶入會員與 User ID，然後可直接輸入點數進行折抵。",
                    "After scanning, the member and User ID will be filled in automatically so you can enter points right away.",
                    "読み取り後は会員と User ID が自動入力され、すぐにポイントを入力して利用できます。",
                    "스캔 후 회원과 User ID가 자동 입력되어 바로 포인트를 입력해 차감할 수 있습니다.",
                  )}
                </div>
              </div>
            </div>
