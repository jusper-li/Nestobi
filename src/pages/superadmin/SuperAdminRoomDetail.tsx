    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <section className="space-y-6">
          <div className="overflow-hidden rounded-2xl bg-white shadow-sm">
            ...
            <div className="border-t border-gray-100 p-6">
              <div className="flex flex-wrap items-center gap-3">
                ...
                {room.weekend_price ? <span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700">週末 {formatCurrency(room.weekend_price)}</span> : null}
              </div>

              <div className="mt-4 grid gap-3 md:grid-cols-3">
                <div className="rounded-xl bg-gray-50 p-4">
                  <p className="text-xs font-medium text-gray-500">入住人數</p>
                  <p className="mt-1 text-lg font-semibold text-gray-900">{room.min_capacity || room.capacity} - {room.capacity} 人</p>
                </div>
                <div className="rounded-xl bg-gray-50 p-4">
                  <p className="text-xs font-medium text-gray-500">樓層</p>
                  <p className="mt-1 text-lg font-semibold text-gray-900">{room.floor || '-'}</p>
                </div>
                <div className="rounded-xl bg-gray-50 p-4">
                  <p className="text-xs font-medium text-gray-500">地點</p>
                  <p className="mt-1 text-lg font-semibold text-gray-900">{room.location || '-'}</p>
                </div>
              </div>

              <div className="mt-4 prose max-w-none text-gray-700">
                <p>{room.description || '尚未提供住宿說明'}</p>
              </div>
            </div>
          </div>
          ...
          {images.length > 0 && (
            <div className="rounded-2xl bg-white p-6 shadow-sm">
              <h2 className="mb-4 text-lg font-bold text-gray-900">住宿圖片</h2>
          ...
          <div className="rounded-2xl bg-white p-6 shadow-sm">
            <h2 className="mb-4 text-lg font-bold text-gray-900">設施與服務</h2>
            <div className="flex flex-wrap gap-2">
              {(room.amenities || []).length ? (
                ...
                <span className="text-sm text-gray-400">尚未提供設施資料</span>
              )}
            </div>
          </div>
        </section>
        ...
        <aside className="space-y-6">
          <div className="rounded-2xl bg-white p-6 shadow-sm">
            <h2 className="mb-4 text-lg font-bold text-gray-900">管理資訊</h2>
            <div className="space-y-4 text-sm">
              ...
              <div>
                <p className="text-xs font-medium text-gray-500">住宿館別</p>
                {room.hotels?.id ? (
              ...
              <div>
                <p className="text-xs font-medium text-gray-500">城市</p>
              ...
              <div>
                <p className="text-xs font-medium text-gray-500">最後更新</p>
