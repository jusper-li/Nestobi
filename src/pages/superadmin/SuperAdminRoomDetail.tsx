    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
        <section className="space-y-6">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <span
                  className={`rounded-full px-3 py-1 text-xs font-semibold ${
                    room.is_available ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'
                  }`}
                >
                ...
                {room.weekend_price ? (
                  <span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-semibold text-blue-700">
                    週末 {formatCurrency(room.weekend_price)}
                  </span>
                ) : null}
              </div>
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-3">
                <div className="rounded-xl bg-gray-50 p-4">
                  ...
                  <p className="mt-1 text-lg font-semibold text-gray-900">
                    {room.min_capacity || room.capacity} - {room.capacity} 人
                  </p>
                </div>
                <div className="rounded-xl bg-gray-50 p-4">
          ...
          <div className="rounded-2xl bg-white p-6 shadow-sm">
            <h2 className="mb-4 text-lg font-bold text-gray-900">設施與備註</h2>
            <div className="flex flex-wrap gap-2">
              {(room.amenities || []).length ? (
                ...
                <span className="text-sm text-gray-400">尚未設定設施</span>
              )}
            </div>
          </div>
        </section>
        ...
        <aside className="space-y-6">
          <div className="rounded-2xl bg-white p-6 shadow-sm">
            ...
            <div className="space-y-4 text-sm">
              <div>
                ...
                {room.vendors?.id ? (
                  <Link
                    to={`/superadmin/vendors/detail/${room.vendors.id}`}
              ...
              <div>
                <p className="text-xs font-medium text-gray-500">飯店</p>
                {room.hotels?.id ? (
                  <Link
                    to={`/superadmin/hotels/detail/${room.hotels.id}`}
                    className="mt-1 inline-flex items-center gap-1 font-semibold text-amber-700 hover:underline"
                  >
          ...
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm text-amber-900">
            ...
            <p className="mt-2 leading-6">
              這裡集中顯示房型的價格、可住人數、設施與關聯資料，方便後台快速檢查與維護。
            </p>
          </div>
        </aside>
      </div>
