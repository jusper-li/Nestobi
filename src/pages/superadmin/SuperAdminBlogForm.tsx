...
const SuperAdminBlogForm: React.FC = () => {
  ...
  const applyScraperResult = () => {
    ...
    const cat = categoryNames.includes(scraperResult.category) ? scraperResult.category : categoryNames[0] || '';
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-4">
        <button onClick={() => navigate('/superadmin/blog')} className="p-2 rounded-xl hover:bg-gray-100 transition text-gray-500">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <div className="flex items-center justify-between mb-3">
              <label className="text-sm font-semibold text-gray-700">
                文章內容（支援 HTML）<span className="text-red-500">*</span>
              </label>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-400"
              >
                {categories.map(c => <option key={c.id} value={c.name}>{getCategoryOptionLabel(c, categories)}</option>)}
              </select>
            </div>
            <div>
