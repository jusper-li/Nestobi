...
const navLinks = [
  { to: '/superadmin', icon: <LayoutDashboard className="w-5 h-5" />, label: '超級後台首頁', end: true },
  { to: '/superadmin/products', icon: <ShoppingBag className="w-5 h-5" />, label: '商品管理' },
  { to: '/superadmin/product-categories', icon: <Tags className="w-5 h-5" />, label: '商品分類管理' },
  { to: '/superadmin/orders', icon: <Package className="w-5 h-5" />, label: '訂單管理' },
  { to: '/superadmin/vendors', icon: <Store className="w-5 h-5" />, label: '廠商管理' },
  { to: '/superadmin/rooms', icon: <BedDouble className="w-5 h-5" />, label: '房間管理' },
  { to: '/superadmin/store-locations', icon: <MapPin className="w-5 h-5" />, label: '門市據點' },
  { to: '/superadmin/blog', icon: <Coffee className="w-5 h-5" />, label: '咖啡旅誌' },
  { to: '/superadmin/coffee-quiz', icon: <Coffee className="w-5 h-5" />, label: '咖啡測驗管理' },
  { to: '/superadmin/blog-categories', icon: <FolderOpen className="w-5 h-5" />, label: '文章分類管理' },
  { to: '/superadmin/users', icon: <Users className="w-5 h-5" />, label: '會員管理' },
  { to: '/superadmin/revenue', icon: <BarChart2 className="w-5 h-5" />, label: '營收分析' },
  { to: '/superadmin/ai-analytics', icon: <Brain className="w-5 h-5" />, label: 'AI 使用分析' },
  { to: '/superadmin/chatbot', icon: <MessageSquare className="w-5 h-5" />, label: 'AI 對話設定' },
  { to: '/superadmin/static-pages', icon: <FileText className="w-5 h-5" />, label: '靜態頁面' },
  { to: '/superadmin/permissions', icon: <Shield className="w-5 h-5" />, label: '權限管理' },
  { to: '/superadmin/faq', icon: <HelpCircle className="w-5 h-5" />, label: 'FAQ 管理' },
