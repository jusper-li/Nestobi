...
export default function SuperAdminBlogCategories() {
  ...
  const handleDelete = async (category: CategoryRow) => {
    ...
    const confirmed = window.confirm(
      usedCount > 0
        ? `${category.name} 目前連到 ${usedCount} 篇文章。刪除後文章不會刪除，但會移除此分類關聯，確定要繼續？`
        : `確定要刪除 ${category.name}？`,
    );
    if (!confirmed) return;
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">文章分類管理</h1>
            <p className="text-sm text-gray-500">管理文章大分類與次分類，前台咖啡旅誌會同步套用。</p>
          </div>
        </div>
        <button
          type="button"
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_400px]">
              ...
              <table className="w-full text-sm">
                <thead className="border-b border-gray-100 bg-gray-50 text-xs font-semibold text-gray-500">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-right">主要分類</th>
                    <th className="px-4 py-3 text-right">關聯文章</th>
        ...
        <motion.form initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} onSubmit={handleSubmit} className="rounded-2xl bg-white p-5 shadow-sm">
          ...
          <div className="space-y-4">
            ...
            <label className="block">
              <span className="mb-1 block text-sm font-medium text-gray-700">描述</span>
