...
export default function HotelDetail() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      const [h, r] = await Promise.all([
        ...
        supabase
          .from('tbl_rooms')
          .select('id,name,description,room_type,capacity,min_capacity,price_per_night,weekend_price,image_url,is_available')
          .eq('hotel_id', id)
  ...
  useEffect(() => {
    ...
    setNotice(
      runtime.tableUnavailable || runtime.isLocalProxyMode
        ...
        : t('套用翻譯快取中...', 'Applying cached translations...', '翻訳キャッシュを適用中...', '번역 캐시 적용 중...'),
    );
    Promise.all([translateHotelsOnDemand([hotel], locale), translateRoomsFromCacheOnly(rooms, locale)])
      .then(([h, r]) => {
        if (cancelled) return;
        setDisplayHotel((h[0] || hotel) as Hotel);
        setDisplayRooms(r);
        setNotice('');
      })
      .catch(() => {});
  ...
  }, [hotel, rooms, locale, shouldTranslate, t]);
  ...
  if (!currentHotel) return <div className="min-h-screen bg-gray-50"><Navigation /><div className="py-24 text-center">{t('找不到飯店', 'Hotel not found', 'ホテルが見つかりません', '호텔을 찾을 수 없습니다')}</div></div>;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="mx-auto max-w-6xl px-4 py-6">
        ...
        <Link to="/rooms" className="mb-4 inline-flex items-center gap-2 text-sm text-gray-600"><ArrowLeft className="h-4 w-4" />{t('返回住宿列表', 'Back to Stays', '宿泊一覧へ戻る', '숙소 목록으로 돌아가기')}</Link>
        ...
        <p className="mt-4 text-gray-700">{currentHotel.description || t('尚無說明', 'No description yet.', '説明はまだありません。', '아직 설명이 없습니다.')}</p>

        <h2 className="mb-4 mt-8 text-2xl font-bold">{t('可預訂房型', 'Available Rooms', '予約可能な客室', '예약 가능한 객실')}</h2>
          ...
          {currentRooms.map(room => {
            ...
            return (
              <article key={room.id} className="rounded-2xl border bg-white p-4 shadow-sm">
                ...
                <p className="mt-1 inline-flex items-center gap-1 text-sm text-gray-500"><Users className="h-4 w-4" />{capacityText} {t('人', 'guests', '名', '명')}</p>
                <p className="mt-2 line-clamp-2 text-sm text-gray-600">{room.description || t('尚無說明', 'No description yet.', '説明はまだありません。', '아직 설명이 없습니다.')}</p>
                <div className="mt-3 text-3xl font-bold text-[#2C1F10]">{formatCurrency(room.price_per_night)}</div>
                {room.weekend_price ? <p className="text-sm text-gray-500">{t('假日', 'Weekend', '週末', '주말')} {formatCurrency(room.weekend_price)}</p> : null}
                <Link to={`/rooms/${room.id}`} className="mt-3 inline-block rounded-xl bg-[#C09A6A] px-4 py-2 text-white">{t('查看詳細', 'Details', '詳細を見る', '상세 보기')}</Link>
              </article>
            );
          })}
        </div>
