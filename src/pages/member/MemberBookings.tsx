...
export default function MemberBookings() {
  ...
  const handleCancel = async (id: string) => {
    const confirmText = pick(locale, '確定要取消這筆訂房嗎？', 'Cancel this booking?', 'この予約をキャンセルしますか？', '이 예약을 취소하시겠습니까?');
  ...
  return (
    <div className="space-y-4">
      <h2 className="flex items-center gap-2 text-xl font-bold text-gray-900">
        ...
        {pick(locale, '我的訂房', 'My Bookings', '予約', '내 예약')}
      </h2>

      {bookings.length === 0 ? (
        <div className="rounded-2xl bg-white p-12 text-center shadow-sm">
          <BedDouble className="mx-auto mb-4 h-14 w-14 text-gray-200" />
          <p className="text-gray-400">{pick(locale, '目前沒有訂房資料', 'No bookings yet', '予約はまだありません', '예약 내역이 없습니다')}</p>
        </div>
      ) : (
          ...
          <motion.div key={booking.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className="overflow-hidden rounded-2xl bg-white shadow-sm">
            <div className="flex flex-col gap-0 sm:flex-row">
              <img
                ...
                alt={booking.tbl_rooms?.name || pick(locale, '房型', 'Room', '部屋', '객실')}
                className="h-32 w-full flex-shrink-0 object-cover sm:h-auto sm:w-36"
              />
              <div className="flex-1 p-5">
                <div className="mb-3 flex flex-wrap items-start justify-between gap-2">
                  <div>
                    <h3 className="font-semibold text-gray-900">{booking.tbl_rooms?.name || pick(locale, '房型', 'Room', '部屋', '객실')}</h3>
                ...
                <div className="mb-3 flex flex-wrap gap-4 text-sm text-gray-600">
                  <div className="flex items-center gap-1">
                    ...
                    {formatDate(booking.check_in_date, dateLocale)} ~ {formatDate(booking.check_out_date, dateLocale)}
                  </div>
                  <div className="flex items-center gap-1">
                    <Users className="h-4 w-4 text-gray-400" />
                    {booking.guests} {pick(locale, '人', 'guests', '名', '명')}
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  {booking.status === 'confirmed' && (
                    ...
                    >
                      {cancelId === booking.id
                        ? pick(locale, '取消中...', 'Cancelling...', 'キャンセル中...', '취소 중...')
                        : pick(locale, '取消訂房', 'Cancel Booking', '予約をキャンセル', '예약 취소')}
                    </button>
                  )}
                </div>
              </div>
