...
export default function Points() {
  ...
  const rewards = [
    {
      points: 100,
      title: pick(locale, 'NT$10 折價券', 'NT$10 Coupon', 'NT$10 クーポン', 'NT$10 쿠폰'),
      description: pick(locale, '可兌換 NT$10 折價券', 'Redeem NT$10 coupon', 'NT$10 クーポンに交換', 'NT$10 쿠폰으로 교환'),
    },
    {
      points: 300,
      title: pick(locale, 'NT$35 折價券', 'NT$35 Coupon', 'NT$35 クーポン', 'NT$35 쿠폰'),
      description: pick(locale, '可兌換 NT$35 折價券', 'Redeem NT$35 coupon', 'NT$35 クーポンに交換', 'NT$35 쿠폰으로 교환'),
    },
    {
      points: 500,
      title: pick(locale, '免費早餐', 'Free Breakfast', '朝食無料', '무료 조식'),
      description: pick(locale, '訂房可加贈免費早餐', 'Free breakfast add-on for booking', '宿泊予約時に無料朝食を追加', '숙박 예약 시 무료 조식 추가'),
    },
  ];
  ...
  const handleRedeem = async (reward: (typeof rewards)[0], index: number) => {
    ...
    const { error } = await supabase.from('points').insert({
      ...
      description: `${pick(locale, '兌換', 'Redeemed', '交換', '교환')}: ${reward.title}`,
    });
    ...
    <div className="space-y-6">
      <AnimatePresence>
        {redeemSuccess && (
          ...
          >
            ...
            <span>
              {pick(locale, '兌換成功', 'Redeemed', '交換完了', '교환 완료')}: {redeemSuccess}
            </span>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl bg-gradient-to-r from-yellow-400 to-orange-400 p-6 text-white">
        <div className="mb-2 flex items-center gap-3">
          ...
          <h2 className="text-xl font-bold">{pick(locale, '我的點數', 'My Points', 'マイポイント', '내 포인트')}</h2>
        </div>
        <p className="mb-1 text-5xl font-bold">{balance.toLocaleString()}</p>
        <p className="text-sm text-yellow-100">{pick(locale, '可使用點數', 'Available points', '利用可能ポイント', '사용 가능 포인트')}</p>
      </motion.div>

      <div>
        <h3 className="mb-4 flex items-center gap-2 text-lg font-bold text-gray-900">
          <Gift className="h-5 w-5 text-orange-500" />
          {pick(locale, '獎勵中心', 'Reward Center', '特典センター', '리워드 센터')}
        </h3>
              ...
              >
                ...
                <div className="mt-3 flex items-center justify-between">
                  <span className="text-sm font-bold text-orange-500">
                    {r.points.toLocaleString()} {pick(locale, '點', 'pts', 'pt', '점')}
                  </span>
                  {isRedeeming ? (
                    <div className="h-4 w-4 animate-spin rounded-full border-2 border-orange-400 border-t-transparent" />
                  ) : canRedeem ? (
                    <span className="rounded-full bg-yellow-100 px-2 py-0.5 text-xs text-yellow-700">{pick(locale, '立即兌換', 'Redeem now', '今すぐ交換', '즉시 교환')}</span>
                  ) : (
                    <span className="text-xs text-gray-400">
                      {pick(locale, '尚需', 'Need', 'あと', '부족')} {(r.points - balance).toLocaleString()} {pick(locale, '點', 'pts', 'pt', '점')}
                    </span>
                  )}
                </div>
              </motion.div>
      ...
      <div>
        <h3 className="mb-4 text-lg font-bold text-gray-900">{pick(locale, '點數紀錄', 'Point History', 'ポイント履歴', '포인트 내역')}</h3>
        {transactions.length === 0 ? (
          <div className="rounded-2xl bg-white p-10 text-center text-gray-400 shadow-sm">
            ...
            <p>{pick(locale, '目前沒有點數紀錄', 'No point transactions yet', 'ポイント履歴はまだありません', '포인트 내역이 없습니다')}</p>
          </div>
        ) : (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-900">
                      {tx.description || pick(locale, '點數異動', 'Point transaction', 'ポイント変動', '포인트 변동')}
                    </p>
