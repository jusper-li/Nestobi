...
export default function EditorialHome({ banner, banners, bannerIndex, setBannerIndex, rooms, products, posts, search, setSearch, onSearch, labels, dateLocale, fallbackRoom, fallbackProduct, fallbackBlog, onImageError }: Props) {
  ...
  return (
    <main className="bg-[#F4F1EC] text-[#24231F]">
      ...
      {(rooms.length > 0 || products.length > 0 || posts.length > 0) && <section className="px-6 py-20 sm:px-10"><div className="mx-auto max-w-[1440px]"><div className="mb-8 flex items-end justify-between"><div><p className="text-xs tracking-[0.28em] text-[#69665F]">FROM THE JOURNEY</p><p className="mt-3 max-w-xl text-sm leading-7 text-[#69665F]">{labels.translationNotice}</p></div><Link to="/shop" className="text-xs tracking-[0.15em] text-[#69665F] underline underline-offset-8">{labels.explore}</Link></div><div className="grid gap-8 md:grid-cols-3">{rooms.slice(0, 1).map(room => <Link key={room.id} to={`/rooms/${room.id}`} className="group"><img src={room.images?.[0] || room.image_url || fallbackRoom} alt={room.name} onError={event => onImageError(event, fallbackRoom)} className="aspect-[4/3] w-full object-cover" /><h3 className="mt-4 text-lg">{room.name}</h3><p className="mt-2 text-xs text-[#69665F]">{room.capacity} {labels.guests} · {formatPrice(room.price_per_night)} {labels.perNight}</p></Link>)}{products.slice(0, 1).map(product => <Link key={product.id} to={`/shop/${product.id}`} className="group"><img src={product.image_url || fallbackProduct} alt={product.name} onError={event => onImageError(event, fallbackProduct)} className="aspect-[4/3] w-full object-cover" /><h3 className="mt-4 text-lg">{product.name}</h3><p className="mt-2 text-xs text-[#69665F]">{formatPrice(product.price)}</p></Link>)}{posts.slice(0, 1).map(post => <Link key={post.id} to={`/blog/${post.slug}`} className="group"><img src={post.cover_image_url || fallbackBlog} alt={post.title} onError={event => onImageError(event, fallbackBlog)} className="aspect-[4/3] w-full object-cover" /><h3 className="mt-4 text-lg">{post.title}</h3><p className="mt-2 text-xs text-[#69665F]">{new Date(post.published_at).toLocaleDateString(dateLocale)} · {post.category}</p></Link>)}</div></div></section>}
    </main>
  );
}
