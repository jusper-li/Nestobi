...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const postData = {
      ...
      SearchType: invoice.invoice_number && invoice.invoice_random_number ? "0" : "1",
      MerchantOrderNo: String(invoice.ezpay_raw_request?.postData?.MerchantOrderNo || ""),
      TotalAmt: String(Math.round(Number(invoice.total_amount || 0))),
      InvoiceNumber: String(invoice.invoice_number || ""),
      RandomNum: String(invoice.invoice_random_number || ""),
      DisplayFlag: "0",
    };
    ...
    await supabase
      ...
      .update({
        ...
        error_message: trimText(parsed.Message || parsed.message) || null,
      })
      .eq("order_id", order_id);
    ...
    console.error("[ezpay-invoice-query] Error:", error);
    return jsonResponse({
      success: false,
      error: error instanceof Error ? error.message : "Invoice query failed.",
    }, 500);
  }
});
