  ...
  WITH source_documents AS (
    ...
    SELECT
      ...
      concat_ws(E'\n',
        ...
        '可入住人數: ' || coalesce(tbl_rooms.capacity::text, ''),
        '每晚價格: ' || coalesce(tbl_rooms.price_per_night::text, ''),
        '地點: ' || coalesce(tbl_rooms.location, ''),
        '設施: ' || coalesce(tbl_rooms.amenities::text, ''),
        '介紹: ' || regexp_replace(coalesce(tbl_rooms.description, ''), '<[^>]+>', ' ', 'g'),
        '詳情連結: /rooms/' || tbl_rooms.id::text,
    ...
    SELECT
      ...
      concat_ws(E'\n',
        ...
        '產地: ' || coalesce(products.origin, ''),
        '烘焙度: ' || coalesce(products.roast_level, ''),
        '處理法: ' || coalesce(products.processing_method, ''),
        '風味筆記: ' || coalesce(products.flavor_notes::text, ''),
        '標籤: ' || coalesce(products.tags::text, ''),
        '介紹: ' || regexp_replace(coalesce(products.description, ''), '<[^>]+>', ' ', 'g'),
      ...
      jsonb_build_object(
        ...
        'processing_method', products.processing_method,
        'flavor_notes', products.flavor_notes,
        'tags', products.tags
      ) AS metadata
...
CREATE OR REPLACE FUNCTION public.match_ai_support_documents_by_type(
  query_embedding text,
  source_types text[] DEFAULT ARRAY['room', 'product', 'article', 'store', 'faq'],
...
AS $$
  ...
  WHERE documents.is_active = true
    AND documents.source_type = ANY(source_types)
...
REVOKE ALL ON FUNCTION public.match_ai_support_documents_by_type(text, text[], integer, double precision) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.match_ai_support_documents_by_type(text, text[], integer, double precision) TO service_role;
