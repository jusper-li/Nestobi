/*
  # Require shipping info for shop checkout orders

  - Add a saved shipping address field to member profiles so checkout can
    prefill the next order automatically.
  - Require name, phone, and address when creating shop checkout orders.
  - Persist the latest shipping info back to the member profile and onto the
    order snapshots for vendor staff.
*/
...
BEGIN
  ...
  v_customer_name := trim(coalesce(p_shipping_name, ''));
  v_customer_phone := trim(coalesce(p_shipping_phone, ''));
  v_shipping_address_text := trim(coalesce(p_shipping_address, ''));

  IF v_customer_name = '' OR v_customer_phone = '' OR v_shipping_address_text = '' THEN
  ...
  v_shipping_address := jsonb_build_object(
    ...
    'address', v_shipping_address_text,
    'shipping_address', v_shipping_address_text,
    'recipient_address', v_shipping_address_text
  );

  INSERT INTO public.tbl_mn5wgzh0 (
    user_id,
    display_name,
    phone,
    shipping_address,
...
GRANT EXECUTE ON FUNCTION public.create_shop_checkout_order(text, text, text, text, integer)
  TO authenticated;
